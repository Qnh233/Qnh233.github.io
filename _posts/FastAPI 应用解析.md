...
# 前言 - 异步（asyncio）
简单拆成 3 个核心组件：
1. **事件循环 event loop**：调度器，维护 2 个队列
    - 就绪队列：已经可以马上执行的协程
    - IO 等待队列：在等 IO（网络、socket）完成的协程
2. **IO 多路复用（select/epoll/kqueue）** —— 异步的基石
    操作系统提供的能力：一次性监听一堆 socket 网络连接。
    事件循环调用 epoll，**阻塞在操作系统这一层**，等待任意一个 socket 有数据返回。
    在等待 epoll 返回期间，Python 主线程这里完全空闲。
3. **协程**：可挂起恢复的任务，遇到 await 就交出控制权。

完整流程举个网络请求例子：

1. 协程 A 发起网络请求，`await http_get()`
2. await 触发：协程 A 暂停，保存当前状态，把 socket 交给 epoll 监听，**把执行权还给事件循环**
3. 事件循环看就绪队列，发现协程 B 可以跑，就去执行协程 B
4. 协程 B 也遇到 await，暂停，交还控制权
5. 就绪队列空了 → 事件循环调用 epoll，阻塞等待 OS：看看哪个 socket 有数据回来了
6. 过一会，网络返回数据，OS 通知 epoll：socketA 就绪
7. 事件循环收到通知，把协程 A 放回就绪队列，下次轮到它执行时，从 await 后面继续跑代码

> 核心：**等待 IO 的时候，Python 主线程不是在忙等，而是交给操作系统 epoll 去监听，CPU 空闲，去处理别的协程。**

这就是异步高并发的来源：**大量 IO 等待时间，主线程不浪费，可以处理别的任务**。
...
- `await asyncio.sleep()`：异步，遇到await，协程主动挂起，交还控制权给事件循环，不阻塞其他任务 
- 协程：**可暂停、可恢复的函数**，`async def` 定义协程；遇到 `await` 主动交出执行权（非抢占式！代码不await，事件循环无法打断它）。 
- 事件循环：单线程调度器，配合epoll IO多路复用，在IO等待期间去处理其他协程，实现高并发。 
- ✅ 异步高并发本质：**复用IO等待的空闲时间，不是多核并行计算**。 
